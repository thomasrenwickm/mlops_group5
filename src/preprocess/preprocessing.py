...
def clean_raw_data(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    logger.info("Dropped rows with missing values in: %s", dropna)
...
def build_preprocessing_pipeline(config: dict,
    ...
    if encoding_cfg:
        ohe = OneHotEncoder(handle_unknown=encoding_cfg.get("handle_unknown",
                                                            "ignore"),
    ...
    if scale_cfg:
        ...
        scaler = (
            StandardScaler() if scale_type == "standard" else MinMaxScaler()
                   )
...
def transform_with_pipeline(df: pd.DataFrame,
                            config: dict,
                            pipeline: ColumnTransformer) -> pd.DataFrame:
